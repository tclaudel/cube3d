...
/*   Updated: 2020/01/13 10:22:14 by tclaudel    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static void		ft_raycast_set_draw(t_cub *c)
{
	...
	if (c->draw_sprite_end.y >= c->res[0])
	...
	c->draw_sprite_end.x = c->sprite_height / 2 + c->sprite_screen_x;
...
void			ft_raycast_sprites(t_cub *c)
{
	...
	while ((int)i < c->nb_sprites)
	{
		...
		c->transform.x = c->inv_det *
			(c->dir.y * c->sprite.x - c->dir.x * c->sprite.y);
		c->transform.y = c->inv_det *
			(-c->plane.y * c->sprite.x + c->plane.x * c->sprite.y);
		c->sprite_screen_x = (int)(c->res[0] / 2) *
			(1 + c->transform.x / c->transform.y);
		c->sprite_height = ft_abs((int)(c->res[1] / c->transform.y));
		c->draw_sprite_start.y = -c->sprite_height / 2 + c->res[1] / 2;
